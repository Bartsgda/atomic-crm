...
export const ClientFormModal: React.FC<Props> = ({ isOpen, onClose, onSave, initialData, aiDiffs = {}, onUserCorrect }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-md">
      <div className="bg-white dark:bg-zinc-900 w-full max-w-4xl h-[90vh] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border border-zinc-200 dark:border-zinc-800">
        ...
        <div className="p-6 md:p-8 border-t border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-950 flex justify-between items-center">
            ...
            <div className="flex gap-4">
                <button onClick={onClose} className="px-8 py-3 rounded-xl text-[11px] font-black uppercase text-zinc-500 hover:bg-zinc-100 transition-all">Anuluj</button>
                <button 
                    onClick={() => document.getElementById('client-form')?.dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }))}
                    className="px-10 py-3 bg-zinc-900 text-white rounded-xl text-[11px] font-black uppercase flex items-center gap-2 shadow-xl hover:bg-black transition-all tracking-widest"
                >
