...
export const ClientsList: React.FC<Props> = ({ state, onNavigate, onSaveClient, onImportComplete, isCompact = false, initialAutoCreate = false }) => {
  ...
  const processedList = useMemo(() => {
      const sourceList = showCoOwners ? coOwnerList : state.clients;
  ...
  }, [state.policies, state.clients, state.notes, coOwnerList, showCoOwners, searchTerm, sortKey, sortDir]);
    ...
    <div className="p-6 md:p-10 max-w-[1920px] mx-auto select-none h-full flex flex-col">
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6 mb-6 flex-shrink-0">
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto items-center">
            {/* SWITCH */}
            <div className="flex bg-zinc-200 dark:bg-zinc-800 p-1 rounded-xl flex-shrink-0">
                <button 
                    onClick={() => setShowCoOwners(false)}
                    className={`px-4 py-2 rounded-lg text-xs font-black uppercase transition-all ${!showCoOwners ? 'bg-white dark:bg-zinc-600 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}
                >
                    Klienci
                </button>
                <button 
                    onClick={() => setShowCoOwners(true)}
            ...
            <div className="relative flex-1 md:w-[400px] group">
              ...
              <input 
                type="text" 
                className="block w-full pl-12 pr-10 py-3 border-2 border-zinc-200 dark:border-zinc-700 rounded-2xl text-zinc-900 dark:text-white focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none font-bold shadow-sm bg-white dark:bg-zinc-900 transition-all text-sm" 
            ...
            {/* ACTION */}
            {!showCoOwners && (
                <button onClick={openNewClient} className="flex items-center gap-2 px-6 py-3 bg-zinc-950 dark:bg-zinc-700 text-white rounded-2xl text-sm font-black uppercase shadow-xl hover:bg-black dark:hover:bg-zinc-600 transition-all tracking-widest whitespace-nowrap justify-center">
      ...
      {/* TABLE */}
      <div className={`flex-1 bg-white dark:bg-zinc-900 border-2 ${showCoOwners ? 'border-purple-200 dark:border-purple-900/30' : 'border-zinc-100 dark:border-zinc-800'} rounded-[2.5rem] shadow-2xl overflow-hidden transition-colors duration-500 flex flex-col`}>
          ...
          <table className="w-full text-left border-collapse">
            <thead className={`sticky top-0 z-10 bg-zinc-50/95 dark:bg-zinc-950/95 backdrop-blur-sm border-b-2 ${showCoOwners ? 'border-purple-100' : 'border-zinc-100 dark:border-zinc-800'} text-[10px] uppercase font-black text-zinc-400 tracking-widest`}>
              <tr>
                ...
                <th className={`px-8 ${isCompact ? 'py-3' : 'py-5'} text-right`}>Akcje</th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className={`px-8 ${paddingClass} align-top`}>
                      <div className="flex items-start gap-4">
                        <div className={`${avatarSize} rounded-2xl ${isVirtual ? 'bg-purple-100 text-purple-600' : 'bg-zinc-900 dark:bg-zinc-800 text-white'} flex items-center justify-center font-black shadow-lg group-hover:scale-105 transition-transform mt-1`}>
                          {(client.lastName || 'U')[0]}
                        </div>
                        <div>
                    ...
                    <td className={`px-8 ${paddingClass} text-right align-middle`}>
                      <button 
                        ...
                        className="p-2.5 text-zinc-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-xl transition-all opacity-0 group-hover:opacity-100"
      ...
      {contextMenu && (
        <div 
          className="fixed z-[100] w-64 bg-white dark:bg-zinc-900 border-2 border-zinc-100 dark:border-zinc-800 rounded-3xl shadow-2xl py-3 animate-in fade-in zoom-in-95 duration-150 overflow-hidden" 
