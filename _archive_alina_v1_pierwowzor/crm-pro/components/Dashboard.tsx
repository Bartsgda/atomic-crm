...
const NotesPopover = ({ notes, policyId, position }: { notes: ClientNote[], policyId: string, position: {x:number, y:number} }) => {
    ...
    return (
        <div 
            ...
            className="z-[9999] w-96 bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border-2 border-zinc-100 dark:border-zinc-800 animate-in zoom-in-95 duration-200 pointer-events-none"
        >
            <div className="p-3 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 rounded-t-2xl flex justify-between items-center">
...
export const Dashboard: React.FC<Props> = ({ state, onNavigate, onDeletePolicy, initialSearchTerm, filterTypes, activeCategory, categoryTitle, sortByDate, onImportComplete, isCompact = false, predefinedDateRange }) => {
  ...
  const stats = useMemo(() => {
    ...
    }, { totalPremium: 0, totalCommission: 0, pendingOffers: 0, soldCount: 0 });
  }, [state.policies]);
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1800px] mx-auto space-y-6 bg-zinc-50 dark:bg-zinc-900 min-h-screen relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-zinc-900 rounded-[1.5rem] p-5 border-2 border-zinc-100 dark:border-zinc-800 flex flex-col justify-between shadow-sm relative overflow-hidden group">
              <div className="absolute right-0 top-0 p-4 opacity-5 group-hover:scale-110 transition-transform"><Wallet size={64}/></div>
              <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Prowizja (Total)</p>
              <p className="text-2xl font-black text-zinc-900 dark:text-white mt-1">{stats.totalCommission.toLocaleString()} PLN</p>
          </div>
          <div className="bg-white dark:bg-zinc-900 rounded-[1.5rem] p-5 border-2 border-zinc-100 dark:border-zinc-800 flex flex-col justify-between shadow-sm relative overflow-hidden group">
              <div className="absolute right-0 top-0 p-4 opacity-5 group-hover:scale-110 transition-transform text-amber-500"><Zap size={64}/></div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-[1.5rem] p-5 border-2 border-zinc-100 dark:border-zinc-800 flex flex-col justify-between shadow-sm relative overflow-hidden group">
              <div className="absolute right-0 top-0 p-4 opacity-5 group-hover:scale-110 transition-transform text-emerald-500"><TrendingUp size={64}/></div>
              <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Sprzedaż (Szt.)</p>
              <div className="flex items-center gap-2 mt-1">
                  <p className="text-2xl font-black text-zinc-900 dark:text-white">{stats.soldCount}</p>
                  <span className="text-xs bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded font-bold">Sukces</span>
              </div>
          </div>
          <div className="bg-emerald-600 rounded-[1.5rem] p-5 flex flex-col justify-between shadow-xl shadow-emerald-900/20 text-white relative overflow-hidden group">
              <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:scale-110 transition-transform"><Briefcase size={64}/></div>
              <p className="text-[10px] font-black uppercase text-emerald-200 tracking-widest">Wartość Portfela</p>
              <p className="text-2xl font-black mt-1">{stats.totalPremium.toLocaleString()} PLN</p>
          </div>
      </div>
      
      {/* FILTER BAR */}
      <div className="sticky top-2 z-30">
        <div className="bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-xl transition-all p-3 space-y-2">
      ...
      {/* DATA TABLE */}
      <div className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-xl overflow-hidden flex-1 flex flex-col">
                  ...
                  >
                    ...
                    <td className={`px-6 ${paddingClass}`}>
                      <div className="flex items-center gap-3">
                        <div className={`${avatarSize} rounded-xl bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-zinc-900 dark:text-zinc-100 font-black group-hover:bg-zinc-900 dark:group-hover:bg-zinc-700 group-hover:text-white transition-all border border-zinc-200 dark:border-zinc-700 shadow-sm`}>
                            {client.lastName[0]}
                        </div>
                        <div>
