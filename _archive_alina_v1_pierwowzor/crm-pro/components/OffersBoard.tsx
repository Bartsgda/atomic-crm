...
// ... (Existing Imports and Interfaces remain) ...
...
// ... (Existing Components: MissingDataModal, CelebrationOverlay, OfferPopover remain unchanged) ...
// (I will omit repeating them to save space, assuming they are preserved)
...
const MissingDataModal = ({ policy, fields, onClose, onNavigate }: { policy: Policy, fields: string[], onClose: () => void, onNavigate: any }) => {
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-[2rem] shadow-2xl border-4 border-amber-400 overflow-hidden flex flex-col relative animate-in zoom-in-95 duration-200">
                ...
                <div className="p-8 space-y-6">
                    ...
                    <div className="flex gap-3 pt-2">
                        <button onClick={onClose} className="flex-1 py-3 bg-zinc-100 dark:bg-zinc-800 rounded-xl text-xs font-black uppercase text-zinc-500 hover:text-zinc-900 transition-colors">
                            Uzupełnię Później
                        </button>
                        <button 
                            onClick={() => {
                                onClose();
                                // Znajdź klienta i otwórz edycję polisy
                                onNavigate('client-details', { 
                                    client: { id: policy.clientId }, // Minimal stub logic handles finding real client in app
                                    highlightPolicyId: policy.id,
                                    autoOpenPolicyId: policy.id 
                                });
                            }} 
                            className="flex-1 py-3 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl text-xs font-black uppercase shadow-lg hover:scale-105 transition-transform flex items-center justify-center gap-2"
                        >
...
const CelebrationOverlay = ({ type, onClose }: { type: 'start' | 'sold', onClose: () => void }) => {
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none">
            <div className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-md p-10 rounded-[3rem] shadow-2xl border-4 border-white dark:border-zinc-800 animate-in zoom-in-50 duration-500 flex flex-col items-center gap-6 text-center transform scale-125">
                {type === 'start' ? (
                    <>
...
const OfferPopover: React.FC<OfferPopoverProps> = ({ policy, client, notes, onClose, position }) => {
    // FILTROWANIE NOTATEK SYSTEMOWYCH W PODGLĄDZIE
    // Pokazujemy tylko te, które NIE są systemowe/statusowe, żeby nie śmiecić
    ...
    return (
        <div 
            ...
            className="z-[9999] w-80 bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl border-2 border-zinc-100 dark:border-zinc-700 animate-in zoom-in-95 duration-200 pointer-events-none"
        >
            {/* Header */}
            <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 rounded-t-3xl flex justify-between items-start">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-white dark:bg-zinc-800 flex items-center justify-center shadow-sm text-zinc-500">
                        <User size={18} />
                    </div>
                    <div>
...
export const OffersBoard: React.FC<Props> = ({ state, onNavigate, onRefresh }) => {
  ...
  const [viewMode, setViewMode] = useState<'KANBAN' | 'TABLE'>(() => {
      // Default to TABLE, but respect user choice
  ...
  const handleCardEnter = (e: React.MouseEvent, policy: Policy, client: Client, notes: ClientNote[]) => {
      if (viewMode === 'TABLE') return; // Disable popover in table view
  ...
  // --- STATUS CHANGE & SYSTEM NOTE GENERATION ---
  const handleStatusChange = async (policyId: string, newStage: SalesStage) => {
    ...
    // --- SALES VALIDATION (DATA DEBT LOGIC) ---
    if (newStage === 'sprzedaż') {
        ...
        if (missing.length > 0) {
            // Create Task Note AUTOMATICALLY
            ...
            // Show Custom Modal (Alert) instead of window.alert
    ...
    // --- ANTI-SPAM LOGIC (60 seconds debounce) ---
    ...
    // 2. Celebration (Only if successful sale)
  ...
  const { activeColumns, doneList, grandTotal, tableList } = useMemo(() => {
    // 1. Definicja aktywnych kolumn (Górna część ekranu)
    ...
    // 2. Lista zakończonych (Dolna część ekranu)
    const done: Policy[] = [];
    const tableItems: Policy[] = [];

    state.policies.forEach(p => {
      let key = p.stage;
      if (key === 'of_przedst') key = 'oferta_wysłana'; // Normalize legacy
      ...
      // Logika dla "Recently Done": Sprzedane lub Odrzucone w ciągu ostatnich 7 dni
          ...
          if (lastNote) {
              ...
              // Fallback to update time if no note
              const days = differenceInDays(new Date(), new Date(p.createdAt)); // simplification
              if(days <= 7) done.push(p);
          }
      }
    });
    ...
    // Sort recently done by date desc
    done.sort((a,b) => {
    ...
    // Sort table items by date created
  ...
  return (
    <div className="h-full flex flex-col bg-zinc-50 dark:bg-zinc-950 overflow-hidden font-sans relative">
      ...
      {/* MISSING DATA ALERT (CUSTOM MODAL) */}
      {missingDataAlert && (
          <MissingDataModal 
      ...
      {quickNotePolicy && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-zinc-900/30 backdrop-blur-sm" onClick={() => setQuickNotePolicy(null)}>
              <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl shadow-2xl w-96 border border-zinc-200 dark:border-zinc-800 animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
