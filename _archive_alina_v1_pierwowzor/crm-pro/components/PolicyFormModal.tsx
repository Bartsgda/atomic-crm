...
const OfferComparator = ({ calculations, onUpdate, onSelect, activeList, onAddInsurer }: { 
    ...
    return (
        <div className="bg-white dark:bg-zinc-900 rounded-3xl border-2 border-zinc-100 dark:border-zinc-800 p-4 mb-6 shadow-sm">
    ...
    return (
        <div className="space-y-8 p-8 animate-in fade-in duration-300 max-w-5xl mx-auto">
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <button onClick={onEdit} className="bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 p-4 rounded-2xl flex flex-col items-center justify-center gap-2 shadow-xl hover:scale-[1.02] transition-transform">
                    <Edit size={24} />
                    <span className="text-xs font-black uppercase tracking-widest">Edytuj Dane</span>
                </button>
                
                <button onClick={() => onAction('RENEWAL')} className="bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-2xl flex flex-col items-center justify-center gap-2 shadow-lg hover:scale-[1.02] transition-transform">
                    <RefreshCcw size={24} />
                ...
                {['OC', 'AC', 'BOTH'].includes(policy.type) && (
                    <>
                        <button onClick={() => onAction('SOLD')} className="bg-amber-500 hover:bg-amber-600 text-white p-4 rounded-2xl flex flex-col items-center justify-center gap-2 shadow-lg hover:scale-[1.02] transition-transform">
                            <Key size={24} />
                            <span className="text-xs font-black uppercase tracking-widest">Zgłoś Zbycie</span>
                        </button>
                        <button onClick={() => onAction('TERMINATE')} className="bg-white border-2 border-red-100 text-red-600 p-4 rounded-2xl flex flex-col items-center justify-center gap-2 hover:bg-red-50 hover:border-red-200 transition-all">
                            <FileWarning size={24} />
            ...
            {/* MAIN CARD */}
            <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-8 border-2 border-zinc-100 dark:border-zinc-800 shadow-2xl relative overflow-hidden">
...
const AssetPicker = ({ assets, onSelect }: { assets: Policy[], onSelect: (asset: Policy) => void }) => {
    return (
        <div className="p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/30 mb-6 animate-in slide-in-from-top-4">
...
export const PolicyFormModal: React.FC<Props> = ({ isOpen, onClose, initialClient, clients, initialType, initialPolicy, onSave, existingPolicies = [], notes = [], initialMode, onOpenProfile, aiDiffs: externalAiDiffs, onAddNewClient, renewalSource }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-0 md:p-4 bg-zinc-950/80 backdrop-blur-md">
      ...
      {/* MAIN CONTAINER */}
      <div className={`bg-white dark:bg-zinc-900 rounded-none md:rounded-[2.5rem] shadow-2xl w-full h-full md:h-[90vh] overflow-hidden flex flex-col border border-zinc-200 dark:border-zinc-800 transition-all duration-500 ease-in-out max-w-7xl relative ${isRenewalMode ? 'border-4 border-blue-500/50' : ''}`}>
