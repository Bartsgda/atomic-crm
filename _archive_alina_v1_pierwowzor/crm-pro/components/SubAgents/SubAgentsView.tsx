      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-10 scrollbar-hide">
          <div className="max-w-7xl mx-auto space-y-8">
              ...
              {Object.entries(groupedAgents).map(([groupName, rawData]) => {
                  ...
                  return (
                      <div key={groupName} className="bg-white dark:bg-zinc-900 border-2 border-zinc-100 dark:border-zinc-800 rounded-[2rem] overflow-hidden shadow-sm transition-all hover:shadow-lg">
                                      ...
                                      {data.agents.map(agent => {
                                          ...
                                          return (
                                              <div 
                                                  ...
                                                  className="bg-white dark:bg-zinc-800 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-700 hover:border-blue-400 hover:shadow-md cursor-pointer transition-all group flex flex-col justify-between"
                                              >
                                                  <div>
          ...
          <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="relative group">
                    ...
                    <input 
                        ...
                        className="pl-9 pr-3 py-2 bg-zinc-100 dark:bg-zinc-800 border-transparent focus:bg-white dark:focus:bg-zinc-700 border-2 focus:border-blue-500 rounded-xl text-xs font-bold outline-none transition-all w-48"
                        value={searchTerm}
                ...
                {/* Filters */}
                <div className="flex gap-2">
                    <button onClick={() => setShowOnlyActive(!showOnlyActive)} className={`px-3 py-1.5 rounded-xl border text-[9px] font-black uppercase transition-all flex items-center gap-1 ${showOnlyActive ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-white border-zinc-200 text-zinc-400'}`}>
                        <Activity size={12}/> Aktywni
                    </button>
                    <button onClick={() => setHideZeroPremium(!hideZeroPremium)} className={`px-3 py-1.5 rounded-xl border text-[9px] font-black uppercase transition-all flex items-center gap-1 ${hideZeroPremium ? 'bg-red-50 border-red-200 text-red-600' : 'bg-white border-zinc-200 text-zinc-400'}`}>
