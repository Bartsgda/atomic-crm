...
export const AutoTester: React.FC<Props> = ({ externalOpen, onClose }) => {
  ...
  if (!isOpen) {
      return (
          <button 
            ...
            className="fixed bottom-4 right-4 z-[9999] p-3 bg-zinc-900 text-yellow-400 rounded-full shadow-2xl border-2 border-yellow-500 font-black hover:scale-110 transition-transform flex items-center justify-center group"
            title="Otwórz Konsolę Testera"
          >
              <Bug size={24} className="group-hover:animate-spin" />
          </button>
      )
  }

  return (
    <div className="fixed bottom-4 right-4 z-[9999] w-80 bg-zinc-950 border-2 border-yellow-600 rounded-xl shadow-2xl overflow-hidden font-mono text-xs animate-in slide-in-from-bottom-10 flex flex-col max-h-[80vh]">
        <div className="bg-yellow-600 p-2 flex justify-between items-center flex-shrink-0">
            <span className="font-black text-black flex items-center gap-2">
                <Terminal size={14} /> AGENT-BOT v4.1 (SAFE)
            </span>
        ...
        <style>{`
            .tester-btn {
                @apply w-full flex items-center gap-3 p-2 bg-zinc-900 hover:bg-zinc-800 rounded border border-zinc-800 disabled:opacity-50 text-left transition-colors;
            }
