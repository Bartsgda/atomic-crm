...
import { INSURERS } from '@/crm-pro/towarzystwa'; 
...
class StorageManager {
      ...
      try {
        ...
        if (parsed && Array.isArray(parsed.clients)) {
            ...
            this.state = {
                ...
                checklistTemplates: parsed.checklistTemplates || DEFAULT_CHECKLISTS,
                insurers: syncedInsurers,
                insurerConfigs: parsed.insurerConfigs || {},
                trash: parsed.trash || [] // Init Trash
            };
  ...
  async clearAllData(): Promise<AppState> {
      ...
      this.log('DELETE', 'SYSTEM', 'PEŁNY RESET DANYCH (WIPE)');
      return this.getState();
  }
  ...
  async deletePolicy(id: string): Promise<AppState> {
    ...
    this.log('DELETE', 'POLICY', `Przeniesiono do kosza polisę ${p.insurerName} ${p.vehicleBrand}`, id, p);
    this.save();
    return this.getState();
  }
  ...
  async restoreFromTrash(id: string): Promise<AppState> {
      ...
      if (item.type === 'POLICY') {
          ...
          this.log('RESTORE', 'POLICY', `Przywrócono z kosza: ${item.data.vehicleBrand}`, id);
      } else if (item.type === 'CLIENT') {
          this.state.clients = [...this.state.clients, item.data];
      }
