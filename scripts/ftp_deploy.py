...
def build(sb_pub, sb_sec):
    ...
    UWAGA: klucz Gemini NIE trafia do buildu (byłby publiczny w bundlu). Klucz API jest
    przechowywany ZASZYFROWANY DEK w bazie i odszyfrowywany client-side po podaniu hasła
    aplikacji przez Alinę (patrz services/apiKeyStore.ts + PassphraseGate). Dev (localhost)
