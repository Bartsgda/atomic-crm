...
function App() {
  ...
  >(undefined);
  const [dashboardDateFilter, setDashboardDateFilter] = useState<string | null>(
    null,
  );
  ...
  const handleAgentAction = async (
    action: any,
  ): Promise<{ clientId?: string; policyId?: string } | void> => {
    ...
    if (action.action === "OPEN_MODAL") {
      ...
      if (action.target === "edit-policy" && action.policyId) {
        const policy = stateRef.current.policies.find(
          (p) => p.id === action.policyId,
        );
        if (policy) {
          const client = stateRef.current.clients.find(
  ...
  const handleSaveClient = async (client: Client) => {
    ...
    if (
      ...
      currentData?.client?.id === client.id
    ) {
  ...
  const handlePurgeFromTrash = async (id: string) => {
    if (
      confirm(
        "Czy na pewno chcesz TRWALE usunąć ten element z archiwum? Tej operacji nie da się cofnąć.",
      )
    ) {
  ...
  const handleWipeSystem = async () => {
    if (
      confirm(
        "☢️ NUCLEAR RESET: Czy na pewno chcesz usunąć WSZYSTKIE dane lokalne? Te operacja jest nieodwracalna!",
      )
    ) {
  ...
  const handleWipeData = async () => {
    const confirmText = prompt(
      "☢️ NUCLEAR RESET: usunie WSZYSTKIE dane tenantu w Supabase (klienci, polisy, notatki, kosz). Aby potwierdzić — wpisz: USUŃ WSZYSTKO",
    );
  ...
  return (
    ...
    >
      ...
      {showNotifications && (
        <div
          className="fixed inset-0 z-[200] flex items-start justify-end p-6 bg-zinc-950/20 backdrop-blur-sm"
      ...
      >
        <div className="print:p-0 min-h-full h-full">
          ...
          {currentPage === "finance" && <FinanceView state={state} />}
          ...
          <div className="max-w-md w-full text-center space-y-8">
            ...
            <div className="space-y-2">
              ...
              <p className="text-zinc-400 text-sm">
                Z powodu 8 godzin bezczynności dane zostały usunięte z widoku
                (wymóg RODO). Odśwież stronę, aby kontynuować.
              </p>
            </div>
            <button
