...
export const KaratekaInput: React.FC<Props> = ({ register, diffState, onUserCorrect, className, ...props }) => {
    const [internalValue, setInternalValue] = useState(props.defaultValue || '');
    
    // Determine Visual Style Override if Diff Active
    // If no diff, we use standard classes passed via className or default
    ...
    if (diffState) {
        if (diffState.status === 'PENDING') {
            ...
            visualClass = "w-full p-2.5 rounded-xl text-sm font-black outline-none transition-all border-2 border-purple-500 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 ring-2 ring-purple-500/20";
        } else if (diffState.status === 'MANUAL_OVERRIDE') {
            // User Corrected AI
            visualClass = "w-full p-2.5 rounded-xl text-sm font-black outline-none transition-all border-2 border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 ring-2 ring-emerald-500/20";
        }
    }
        ...
        <div className="relative group">
            ...
            {diffState && diffState.status === 'PENDING' && (
                <div className="absolute -top-5 left-0 text-[9px] flex items-center gap-2 animate-in fade-in slide-in-from-bottom-1 z-10 bg-white dark:bg-zinc-900 px-1 rounded shadow-sm border border-zinc-200 dark:border-zinc-700">
                    <span className="text-zinc-400 line-through decoration-red-500 decoration-2">{diffState.originalValue || '(puste)'}</span>
