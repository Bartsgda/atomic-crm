...
export const ActivityLogView: React.FC<Props> = ({ state, onClose }) => {
  ...
  // Esc zamyka (2026-07-27) — brak propa isOpen, widoczność steruje mount rodzica.
  useEffect(() => {
        ...
        <div className="w-full p-6 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950 flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-black text-zinc-900 dark:text-white flex items-center gap-3">
              <Activity className="text-amber-500" /> Rejestr Czynności (Audit
              Log)
            </h2>
          ...
          <button
            onClick={onClose}
            className="px-6 py-2 bg-zinc-200 dark:bg-zinc-800 rounded-xl text-xs font-black uppercase hover:bg-zinc-300 dark:hover:bg-zinc-700 transition-colors"
          >
            Zamknij
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-0">
          <table className="w-full text-left border-collapse">
            <thead className="bg-zinc-50 dark:bg-zinc-950 sticky top-0 z-10 border-b border-zinc-100 dark:border-zinc-800">
              <tr>
                <th className="px-6 py-3 text-[10px] font-black uppercase text-zinc-400 tracking-widest">
                  Czas
                </th>
                <th className="px-6 py-3 text-[10px] font-black uppercase text-zinc-400 tracking-widest">
                  Akcja
                ...
                <th className="px-6 py-3 text-[10px] font-black uppercase text-zinc-400 tracking-widest w-1/2">
                  Szczegóły
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800">
              {filteredLogs.map((log) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    ...
                    <p className="text-[9px] font-mono text-zinc-400 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      ID: {log.id}
                    </p>
                  </td>
                </tr>
              ...
              {filteredLogs.length === 0 && (
                <tr>
                  <td
                    colSpan={4}
                    className="px-6 py-20 text-center text-zinc-400 font-bold uppercase text-xs"
                  >
