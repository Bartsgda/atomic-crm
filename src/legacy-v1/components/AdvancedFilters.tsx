...
export const AdvancedFilters: React.FC<Props> = ({
  ...
  const getStageIcon = (st: string) => {
    switch (st) {
      ...
      case "of_do zrobienia":
        return <Zap size={14} />;
      case "przeł kontakt":
        return <Clock size={14} />;
      case "oferta_wysłana":
        return <MessageSquare size={14} />;
      case "rez po ofercie_kont za rok":
      ...
      <div className="flex justify-between items-center mb-8 border-b border-zinc-100 dark:border-zinc-800 pb-4 relative z-10">
        <div>
          ...
          <p className="text-[10px] text-zinc-400 font-bold mt-1 uppercase tracking-wide">
            Precyzyjne wyszukiwanie w bazie
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={onClear}
            className="text-[10px] font-black uppercase text-zinc-400 hover:text-red-600 transition-colors px-4 py-2 bg-zinc-50 dark:bg-zinc-800 rounded-xl hover:bg-red-50"
          >
            Wyczyść wszystko
          </button>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-black uppercase text-zinc-600 dark:text-zinc-300 tracking-wide">
              Zakres Czasu
            </p>
          </div>
          ...
          <div className="flex gap-2 mb-3">
            <button
              ...
              className="flex-1 py-2 bg-zinc-100 dark:bg-zinc-800 hover:bg-blue-600 hover:text-white text-zinc-600 dark:text-zinc-400 text-[10px] font-black uppercase rounded-xl transition-all"
            >
              Ten Miesiąc
            </button>
            <button
              onClick={() => applyDatePreset("PREV_MONTH")}
              className="flex-1 py-2 bg-zinc-100 dark:bg-zinc-800 hover:bg-blue-600 hover:text-white text-zinc-600 dark:text-zinc-400 text-[10px] font-black uppercase rounded-xl transition-all"
            >
              Poprzedni
            </button>
        ...
        <div className="lg:col-span-5 space-y-4 border-l border-zinc-100 dark:border-zinc-800 pl-0 lg:pl-8">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              const label = getStatusDisplay(st).label; // nazwa Aliny (Edytor Statusów) lub domyślny label
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    onFilterChange({
                      ...filters,
                      stages: toggleList(filters.stages, st as SalesStage),
                    })
                  }
        ...
        <div className="lg:col-span-3 space-y-4 border-l border-zinc-100 dark:border-zinc-800 pl-0 lg:pl-8">
          <div className="flex items-center gap-2 mb-2">
            <div className="p-1.5 bg-amber-50 dark:bg-amber-900/20 rounded-lg text-amber-600">
              <Layers size={14} />
            </div>
            <p className="text-xs font-black uppercase text-zinc-600 dark:text-zinc-300 tracking-wide">
              Towarzystwo
          ...
          <div className="max-h-40 overflow-y-auto scrollbar-hide border-2 border-zinc-100 dark:border-zinc-800 rounded-2xl p-2 bg-zinc-50/50 dark:bg-zinc-950/50">
            {INSURERS.map((ins) => (
              <label
                key={ins.id}
                className="flex items-center gap-3 p-2 hover:bg-white dark:hover:bg-zinc-800 rounded-xl cursor-pointer group transition-colors"
              >
                <input
                  type="checkbox"
                  ...
                  onChange={() =>
                    onFilterChange({
                      ...filters,
                      insurers: toggleList(filters.insurers, ins.name),
                    })
                  }
                  className="w-4 h-4 rounded border-2 border-zinc-300 text-red-600 focus:ring-red-500"
                />
                <span
                  className={`text-[10px] font-bold uppercase truncate ${filters.insurers.includes(ins.name) ? "text-zinc-900 dark:text-white" : "text-zinc-500"}`}
                >
                  {ins.name}
                </span>
              </label>
