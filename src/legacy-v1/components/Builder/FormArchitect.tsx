...
const INITIAL_LAYOUT: FormElementDef[] = [
  ...
  },
  {
    ...
    label: "Rok Budowy",
    width: "w-1/3",
    bindingKey: "homeDetails.yearBuilt",
  },
  {
    id: "f4",
    type: "SELECT",
    label: "Konstrukcja",
    width: "w-1/3",
    bindingKey: "homeDetails.constructionType",
    options: ["Murowana", "Drewniana"],
  },
];
...
export const FormArchitect: React.FC = () => {
  ...
  // Esc zamyka overlay z wygenerowanym JSON (2026-07-27)
  useEffect(() => {
    if (!jsonOutput) return;
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setJsonOutput("");
    };
            ...
            <div className="bg-white dark:bg-zinc-900 w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
              ...
              <div className="p-4 bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-100 dark:border-zinc-800 text-center">
                <p className="text-[10px] text-zinc-500 font-bold uppercase">
                  Wklej ten kod w czacie, a ja zaktualizuję HomeForm.tsx
                </p>
              </div>
            </div>
          </div>
                ...
                >
                  ...
                  >
                    ...
                    <div className="pointer-events-none">
                      {el.type === "HEADER" ? (
                        <h3 className="text-sm font-black uppercase text-zinc-900 dark:text-white tracking-widest border-b border-zinc-200 pb-1">
                          {el.label}
                        </h3>
                      ) : (
                        <>
                    ...
                    <div
                      className={`absolute top-2 right-2 flex gap-1 ${isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100"} transition-opacity`}
                    >
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className="p-1 bg-white border rounded hover:bg-zinc-100 text-zinc-500"
                        title="Przesuń w lewo/górę"
                      >
                        <MoveUp size={12} />
                      </button>
                      <button
                      ...
                      <button
                        ...
                        className="p-1 bg-white border rounded hover:bg-zinc-100 text-blue-500 font-mono text-[9px] w-12"
                      ...
                      <button
                        ...
                        className="p-1 bg-white border rounded hover:bg-red-50 text-red-500"
                        title="Usuń"
                      >
                        <Trash2 size={12} />
                      </button>
          ...
          {elements.length === 0 && (
            <div className="py-24 text-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl">
              <p className="text-zinc-400 font-bold uppercase text-xs">
                Pusty Formularz. Dodaj elementy z menu po lewej.
              </p>
            </div>
          )}
        </div>
          ...
          {elements.map((el) => {
            ...
            return (
              <div key={el.id} className="p-4 space-y-4">
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase text-zinc-400 block mb-1">
                    Powiązanie (Binding Key)
                  </label>
                  <input
                    value={el.bindingKey || ""}
                    onChange={(e) =>
                      updateElement(el.id, { bindingKey: e.target.value })
                    }
                ...
                <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        updateElement(el.id, { isRequired: e.target.checked })
                      }
