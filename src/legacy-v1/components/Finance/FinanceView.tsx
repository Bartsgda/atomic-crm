  ...
  const yearlyData = useMemo(() => {
    ...
    const stats = months.map((monthIdx) => {
      ...
      policiesInMonth.forEach((p) => {
        ...
        const premium = roundCurrency(
          typeof p.premium === "number"
            ? p.premium
            : parseFloat(String(p.premium)) || 0,
        );
        const subComm = roundCurrency(
          typeof p.subAgentCommission === "number"
            ? p.subAgentCommission
            : parseFloat(String(p.subAgentCommission)) || 0,
        );
        ...
        if (p.subAgentSplits && p.subAgentSplits.length > 0) {
          partnerPart = p.subAgentSplits.reduce(
        ...
        if (
          ...
          p.stage === "ucięty kontakt" ||
          p.stage === "rez po ofercie_kont za rok"
        ) {
      ...
      const conversionRate =
        totalOffers > 0 ? Math.round((countSold / totalOffers) * 100) : 0;
    ...
    const totalNet = roundCurrency(
      stats.reduce((acc, s) => acc + s.incomeNet, 0),
    );
    const totalCost = roundCurrency(
      stats.reduce((acc, s) => acc + s.costPartners, 0),
    );
    const totalGross = roundCurrency(
      stats.reduce((acc, s) => acc + s.revenueGross, 0),
    );
    const totalPotential = roundCurrency(
      stats.reduce((acc, s) => acc + s.potentialLost, 0),
    );

    // Find max value for chart scaling
    const maxChartValue = Math.max(
      ...stats.map((s) => s.incomeNet + s.potentialLost),
      1000,
    );
    ...
    <div className="p-6 md:p-10 min-h-screen bg-zinc-50 dark:bg-zinc-950 font-sans flex flex-col">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-6">
        ...
        <div className="flex items-center bg-white dark:bg-zinc-900 p-1.5 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800">
          <button
            ...
            className="p-3 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-xl text-zinc-500 transition-colors"
          >
            <ChevronLeft size={20} />
          </button>
          <div className="px-6 text-center">
            <span className="block text-xs font-black uppercase text-zinc-400 tracking-widest">
              Rok Obrotowy
            </span>
            <span className="block text-2xl font-black text-zinc-900 dark:text-white tabular-nums">
              {selectedYear}
            </span>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-emerald-600 text-white p-6 rounded-[2rem] shadow-xl shadow-emerald-900/20 relative overflow-hidden group">
          ...
          <p className="text-[10px] font-black uppercase text-emerald-100 tracking-widest mb-1">
            Zysk Netto (Na rękę)
          </p>
          <h2 className="text-4xl font-black tracking-tighter">
            {yearlyData.totalNet.toLocaleString(undefined, {
              minimumFractionDigits: 2,
            })}{" "}
            <span className="text-lg opacity-60">PLN</span>
          </h2>
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm relative group">
          ...
          <h2 className="text-3xl font-black text-zinc-900 dark:text-white tracking-tighter">
            {yearlyData.totalCost.toLocaleString(undefined, {
              minimumFractionDigits: 2,
            })}{" "}
            <span className="text-base text-zinc-400">PLN</span>
          </h2>
          <p className="text-[10px] text-zinc-400 mt-2 font-medium">
            Prowizje wypłacone partnerom
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm relative group">
          ...
          <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest mb-1">
            Przychód Brutto (Total)
          </p>
          <h2 className="text-3xl font-black text-zinc-900 dark:text-white tracking-tighter">
            {yearlyData.totalGross.toLocaleString(undefined, {
              minimumFractionDigits: 2,
            })}{" "}
            <span className="text-base text-zinc-400">PLN</span>
          </h2>
          <p className="text-[10px] text-zinc-400 mt-2 font-medium">
            Suma przypisu prowizji od TU
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm relative group border-t-4 border-t-amber-400">
          ...
          <h2 className="text-3xl font-black text-zinc-400 tracking-tighter decoration-amber-300 decoration-2 line-through">
            {yearlyData.totalPotential.toLocaleString(undefined, {
              minimumFractionDigits: 2,
            })}{" "}
            <span className="text-base">PLN</span>
          </h2>
          <p className="text-[10px] text-zinc-400 mt-2 font-medium">
            Wartość odrzuconych ofert
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-xl p-8 flex flex-col relative overflow-hidden">
        <div className="flex justify-between items-start mb-6">
          <h3 className="text-lg font-black text-zinc-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
            <Calendar size={20} className="text-zinc-400" /> Roczny Wykres
            Sprzedaży
          </h3>
        ...
        {yearlyData.totalGross === 0 && yearlyData.totalPotential === 0 && (
          <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
            <div className="text-center opacity-30">
              ...
              <p className="text-xl font-black text-zinc-500 uppercase">
                Brak danych finansowych
              </p>
              <p className="text-sm font-medium text-zinc-400">
                Dla roku {selectedYear}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex-1 flex items-stretch justify-between gap-2 md:gap-4 relative z-10 min-h-[300px]">
          ...
          {yearlyData.stats.map((month, idx) => {
            // Calculate Heights (%)
            const netHeight = Math.min(
              (month.incomeNet / yearlyData.maxChartValue) * 100,
              100,
            );
            const lostHeight = Math.min(
              (month.potentialLost / yearlyData.maxChartValue) * 100,
              100,
            );
            ...
            return (
              ...
              >
                ...
                {isHovered && (
                  <div className="absolute bottom-[80%] left-1/2 -translate-x-1/2 mb-2 bg-zinc-900 text-white p-3 rounded-xl shadow-xl text-center z-50 animate-in slide-in-from-bottom-2 fade-in w-40 pointer-events-none border border-zinc-700">
                    ...
                    <p className="text-lg font-black text-emerald-400 mb-1">
                      +{month.incomeNet.toFixed(2)} zł
                    </p>
                    {month.potentialLost > 0 && (
                      <p className="text-xs font-bold text-red-400 line-through">
                        -{month.potentialLost.toFixed(2)} zł
                      </p>
                    )}
                ...
                <div className="h-10 text-center w-full">
                  <p
                    className={`text-[10px] font-black uppercase transition-colors ${isHovered ? "text-zinc-900 dark:text-white" : "text-zinc-400"}`}
                  >
                    {month.monthName}
                  </p>
