...
export const TowarzystwaView: React.FC<Props> = ({ state, onRefresh }) => {
  ...
  // Esc zamyka modal "Zarządzaj Listą" (2026-07-27)
  useEffect(() => {
  ...
  const tableData = useMemo(() => {
    ...
    const activeStatic = INSURERS.filter((i) =>
      activeInsurersNames.includes(i.name),
    );

    // Find custom added insurers (in state.insurers but NOT in INSURERS const)
    const customNames = activeInsurersNames.filter(
      (name) => !INSURERS.some((i) => i.name === name),
    );
    ...
    const mergedList = [...activeStatic, ...customInsurers].sort((a, b) =>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-zinc-100 dark:bg-zinc-950 border-b-2 border-zinc-200 dark:border-zinc-800">
                <th className="p-5 text-[10px] font-black uppercase text-zinc-500 tracking-widest w-[30%]">
                  Nazwa / Podmiot
                </th>
                <th className="p-5 text-[10px] font-black uppercase text-zinc-500 tracking-widest w-[25%]">
                  Adres Siedziby
                </th>
                <th className="p-5 text-[10px] font-black uppercase text-zinc-500 tracking-widest w-[25%]">
                  Twój Opiekun
                </th>
                <th className="p-5 text-[10px] font-black uppercase text-zinc-500 tracking-widest text-center w-[10%]">
                  Status
                </th>
                <th className="p-5 text-[10px] font-black uppercase text-zinc-500 tracking-widest text-right w-[10%]">
                  Akcja
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {filteredData.length === 0 ? (
                <tr>
                  <td
                    colSpan={5}
                    className="p-10 text-center text-zinc-400 font-bold uppercase text-xs"
                  >
                    Brak aktywnych towarzystw. Kliknij "Zarządzaj Listą", aby
                    dodać.
                  </td>
                </tr>
              ) : (
                filteredData.map((item, idx) => {
                  ...
                  return (
                    <tr
                      key={idx}
                      className={`transition-colors group ${isEditing ? "bg-blue-50/50 dark:bg-blue-900/10" : "hover:bg-zinc-50/80 dark:hover:bg-zinc-800/30"}`}
                    >
                      {/* Kolumna 1: Nazwa */}
                          ...
                          <div className="space-y-2">
                            {item.email && (
                              <a
                                ...
                                className="flex items-center gap-2 text-xs font-bold text-blue-600 hover:underline decoration-2"
                              >
                            ...
                            {item.managerName ? (
                              ...
                              <span className="text-[9px] text-zinc-300 italic block mt-1">
                                Brak opiekuna
                              </span>
                            )}
                          </div>
                      ...
                      <td className="p-5 align-top text-right">
                        {isEditing ? (
                          <div className="flex flex-col gap-2">
                            <button
                              onClick={saveEditing}
                              className="bg-blue-600 text-white px-2 py-1.5 rounded-lg text-[10px] font-black uppercase hover:bg-blue-700 flex items-center justify-center gap-1"
                            >
                              <Save size={12} /> Zapisz
                            </button>
                            <button
                              onClick={cancelEditing}
                              className="bg-white border border-zinc-200 text-zinc-500 px-2 py-1.5 rounded-lg text-[10px] font-black uppercase hover:bg-zinc-50 flex items-center justify-center gap-1"
                            >
                              <X size={12} /> Anuluj
                            </button>
                          </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-md">
          <div className="bg-white dark:bg-zinc-900 w-full max-w-2xl h-[80vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95">
            <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex justify-between items-center">
              <div>
                <h3 className="text-xl font-black text-zinc-900 dark:text-white flex items-center gap-2">
                  <Settings2 size={24} className="text-blue-600" /> Zarządzaj
                  Listą
                </h3>
                <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mt-1">
                  Wybierz towarzystwa widoczne w systemie
                </p>
              </div>
              <button
                onClick={() => setShowManager(false)}
                className="p-2 bg-zinc-200 dark:bg-zinc-800 rounded-full hover:bg-zinc-300 transition-colors"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-hide">
              {INSURERS.filter((i) =>
                i.name.toLowerCase().includes(managerSearch.toLowerCase()),
              ).map((insurer) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${isActive ? "bg-blue-600 border-blue-600" : "border-zinc-300"}`}
                      >
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-500 font-medium truncate max-w-[200px]">
                          {insurer.currentLegalEntity}
                        </p>
                      </div>
                    </div>
                    {insurer.isBrandOnly && (
                      <span className="text-[9px] font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded">
                        MARKA
                      </span>
                    )}
                  </div>
            ...
            <div className="p-4 bg-zinc-50 dark:bg-zinc-950 border-t border-zinc-100 dark:border-zinc-800">
              <p className="text-[10px] font-black uppercase text-zinc-400 mb-2 pl-1">
