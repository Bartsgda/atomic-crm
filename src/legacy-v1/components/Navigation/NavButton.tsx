...
export const NavButton: React.FC<Props> = ({ icon: Icon, label, count, isActive, onClick, variant = 'default', collapsed }) => {
  ...
  if (collapsed) {
    return (
      <button
        onClick={onClick}
        title={label}
        className={`w-full flex items-center justify-center py-2.5 rounded-xl transition-all mb-1 relative overflow-hidden ${isActive ? activeClass : inactiveClass}`}
      >
        {isActive && (
          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 bg-white rounded-r-full blur-[1px]" />
        )}
        <Icon
          size={19}
