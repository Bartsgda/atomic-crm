...
export const MENU_CATEGORIES = [
  ...
  },
  {
    ...
    label: "Pojazdy",
    icon: Car,
    types: ["OC", "AC", "BOTH"] as PolicyType[],
    sortByDate: false,
    addType: "OC",
  },
  {
...
function sidebarBase(skin: string) {
  if (skin === "luxury-gold") {
    // background/border/shadow handled entirely by v1-themes.css [data-v1-skin="luxury-gold"]
    return "v1-sidebar text-white border-r transition-all duration-700";
  }
...
function sepClass(skin: string) {
  return skin === "default"
    ? "border-t border-zinc-800/50 my-3 mx-4"
    : "v1-sep border-t my-3 mx-4";
}
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const renderNavBtn = (
    ...
    const isActive =
      currentPage === target ||
      (currentPage === "dashboard" && activeCategory === id);
      ...
      <div className="relative group/btn-container" key={`${id}-${label}`}>
        <button
          ...
          style={{
            ...
            borderLeft: isActive
              ? "3px solid var(--v1-active-border)"
        ...
        {(config as any)?.addType && (
          <button
            ...
            style={{
              ...
              color: "var(--v1-accent-fg)",
              borderRadius: "var(--v1-radius-sm)",
            }}
            title={`Dodaj: ${label}`}
          >
    ...
    return (
      ...
      >
        ...
        {count !== undefined && count > 0 && (
          <span className={badgeClass(skin)}>{count}</span>
        )}
      </button>
    );
  };
    ...
    >
      ...
      >
        ...
        {themedNavRow(
          ...
          () => onNavigate("sub-agents"),
          state.subAgents?.length ?? 0,
        )}
        {themedNavRow(Banknote, "Finanse", currentPage === "finance", () =>
          onNavigate("finance"),
        )}
        ...
        {isSystemOpen && (
          <div className="space-y-0.5 pl-2 animate-in slide-in-from-top-2 duration-200">
            {[
              {
                label: "Sahara (Sandbox)",
                icon: Palette,
                page: "sandbox",
                action: () => onNavigate("sandbox"),
              },
              {
                label: "Architekt Formularzy",
                icon: PenTool,
                page: "form-builder",
                action: () => onNavigate("form-builder"),
              },
              {
                label: "Vision Labs",
                icon: Lightbulb,
            ...
            {isAdmin && (
              <div
                className="mt-2 pt-2"
                style={{ borderTop: "1px solid var(--v1-sep, #27272A)" }}
              >
