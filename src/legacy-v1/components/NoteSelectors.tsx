...
// Kolory zsynchronizowane z globalną paletą Aliny (zob. constants.ts -> STATUS_CONFIG).
// Jasne tła (żółty/jasnoszary) dostają ciemny tekst dla kontrastu, reszta biały.
export const SALES_STAGES: { val: SalesStage; label: string; color: string }[] =
  [
    ...
    { val: "sprzedaż", label: "Sprzedane", color: "bg-green-600 text-white" },
    {
      val: "rez po ofercie_kont za rok",
      label: "Rezygnacja",
      color: "bg-slate-300 text-zinc-900",
    },
  ];
    ...
    <div className="flex flex-wrap gap-2 relative">
      ...
      <div className="relative">
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-[10px] font-bold uppercase transition-all shadow-sm border ${curStatus ? curStatus.color + " border-transparent" : "bg-white border-zinc-200 text-zinc-500 hover:border-zinc-300"}`}
        >
          ...
          <div className="absolute bottom-full left-0 mb-2 w-48 bg-white border border-zinc-200 rounded-xl shadow-xl z-[90] py-1 overflow-hidden ring-1 ring-black/5">
            {STATUS_OPTS.map((s) => (
              <button
                ...
                type="button"
                onClick={() => {
                  props.onStatusChange(s.prefix);
                  setOpenMenu(null);
                }}
                className="w-full text-left px-4 py-2.5 text-xs font-medium hover:bg-zinc-50 flex items-center justify-between text-zinc-700"
              >
                {s.label}{" "}
                <div
      ...
      <div className="relative">
        <button
          type="button"
          onClick={() => toggle("mail")}
          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-[10px] font-bold uppercase transition-all shadow-sm border ${curMail ? curMail.color + " border-transparent" : "bg-white border-zinc-200 text-zinc-500 hover:border-zinc-300"}`}
        >
          ...
          <div className="absolute bottom-full left-0 mb-2 w-40 bg-white border border-zinc-200 rounded-xl shadow-xl z-[90] py-1 overflow-hidden ring-1 ring-black/5">
            {MAIL_OPTS.map((o) => (
              <button
                ...
                type="button"
                onClick={() => {
                  props.onMailChange(o.prefix);
                  setOpenMenu(null);
                }}
                className="w-full text-left px-4 py-2.5 text-xs font-medium hover:bg-zinc-50 text-zinc-700 flex items-center gap-2"
              >
                {o.label}
              </button>
            ))}
      ...
      <div className="relative">
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-[10px] font-bold uppercase transition-all shadow-sm border ${curOffer ? curOffer.color + " border-transparent" : "bg-white border-zinc-200 text-zinc-500 hover:border-zinc-300"}`}
        >
      ...
      <div className="relative">
        <button
          type="button"
          onClick={() => toggle("rem")}
          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-[10px] font-bold uppercase transition-all shadow-sm border ${props.activeReminder ? "bg-rose-500 text-white border-rose-600" : "bg-white border-zinc-200 text-zinc-500 hover:border-zinc-300"}`}
        >
          <Clock size={12} />{" "}
          {props.activeReminder
            ? `Za ${props.activeReminder} dni`
            : "Przypomnij"}
        </button>
        {openMenu === "rem" && (
          <div className="absolute bottom-full left-0 mb-2 w-40 bg-white border border-zinc-200 rounded-xl shadow-xl z-[90] py-1 overflow-hidden ring-1 ring-black/5">
