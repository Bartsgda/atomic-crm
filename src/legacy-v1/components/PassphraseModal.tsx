...
export const PassphraseModal: React.FC<Props> = ({
  ...
  const canSubmit =
    mode === "ENTER" ? passphrase.length > 0 : isStrong && matches;

  // Esc zamyka (2026-07-27) — ten dialog to hasło do szyfrowania SYNCHRONIZACJI
  // z chmurą (SET/ENTER), NIE główny ekran blokady aplikacji (to osobny
  // EncryptionGate.tsx/PassphraseGate.tsx, poza zakresem - tamten NIE ma i nie
  // powinien mieć Esc, zob. raport). Ten modal już ma jawny przycisk "Anuluj"
  // wołający dokładnie onClose(), więc Esc jest tylko skrótem do tego samego.
  useEffect(() => {
    if (!isOpen) return;
  ...
  const modalTitle =
    title || (mode === "SET" ? "Zabezpiecz Synchonizację" : "Odszyfruj Dane");
  const modalDesc =
    description ||
    (mode === "SET"
              ...
              <div className="relative">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    placeholder={
                      mode === "SET" ? "Minimum 8 znaków..." : "Wpisz hasło..."
                    }
                    autoFocus
                  />
                  <button
            ...
            {mode === "SET" && (
              <>
                ...
                <div className="p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-100 dark:border-zinc-800 flex flex-wrap gap-x-3 gap-y-1.5">
                  <div
                    className={`flex items-center gap-1.5 text-[9px] font-black ${hasMinLength ? "text-emerald-600" : "text-zinc-400"}`}
                  >
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${hasMinLength ? "bg-emerald-500" : "bg-zinc-300"}`}
                    />
                    8+ ZNAKÓW
                  </div>
                  <div
                    className={`flex items-center gap-1.5 text-[9px] font-black ${hasUppercase ? "text-emerald-600" : "text-zinc-400"}`}
                  >
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${hasUppercase ? "bg-emerald-500" : "bg-zinc-300"}`}
                    />
                    WIELKA LITERA
                  </div>
                  <div
                    className={`flex items-center gap-1.5 text-[9px] font-black ${hasSpecial || hasNumber ? "text-emerald-600" : "text-zinc-400"}`}
                  >
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${hasSpecial || hasNumber ? "bg-emerald-500" : "bg-zinc-300"}`}
                    />
                    CYFRA / ZNAK SPEC.
                  </div>
                </div>
                ...
                <div className="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-xl border border-amber-100 dark:border-amber-900/30 flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-amber-800 dark:text-amber-300 uppercase tracking-widest mb-0.5">
                      Zero-Knowledge
                    </p>
                    <p className="text-[9px] font-bold text-amber-700/80 dark:text-amber-400/80 leading-relaxed">
                      Serwer nigdy nie pozna Twojego hasła. Jeśli je zgubisz,
                      dane w chmurze przepadną na zawsze.
                    </p>
                  </div>
                </div>
              </>
