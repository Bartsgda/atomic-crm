...
export const QuickViewDrawer: React.FC<Props> = ({
  client,
  state,
  onClose,
  onNavigate,
        ...
        <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex justify-between items-start">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-300 font-mono mt-1 font-black tabular-nums tracking-wide">
                {client.pesel || "BRAK PESEL"}
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 hover:bg-white dark:hover:bg-zinc-800 rounded-lg text-zinc-400 transition-colors"
          >
            <X size={20} />
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {client.phones[0] && (
              <a
                ...
                className="flex items-center justify-center gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl font-black text-xs uppercase hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors border border-blue-100 dark:border-blue-900"
              >
              ...
              <div className="space-y-2">
                {pipeline.map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className="p-3 rounded-xl border border-blue-100 dark:border-blue-900/30 bg-blue-50/30 dark:bg-blue-900/10 flex justify-between items-center"
                    >
                      <div>
            ...
            <div className="space-y-2">
              {wallet.length === 0 ? (
                <p className="text-xs text-zinc-400 italic">
                  Brak aktywnych polis.
                </p>
              ) : (
                wallet.map((p) => {
                  const days = differenceInDays(
                    new Date(p.policyEndDate),
                    new Date(),
                  );
                  ...
                  return (
                    <div
                      key={p.id}
                      className={`p-3 rounded-xl border flex justify-between items-center bg-white dark:bg-zinc-900 ${isWarning ? "border-red-200" : "border-zinc-200 dark:border-zinc-800"}`}
                    >
                      <div>
                        <p className="text-xs font-black text-zinc-900 dark:text-zinc-100 uppercase">
                          {p.insurerName}
                        </p>
                        <p className="text-[10px] text-zinc-500 font-medium truncate w-40">
                          {p.vehicleBrand || p.type} {p.vehicleReg}
                        </p>
                      </div>
                      <div className="text-right">
                        <p
                          className={`text-xs font-black ${isWarning ? "text-red-600" : "text-zinc-900 dark:text-zinc-100"}`}
                        >
                          {days} dni
                        </p>
                        <p className="text-[9px] text-zinc-400 font-mono">
                          {p.policyEndDate &&
                          !isNaN(new Date(p.policyEndDate).getTime())
                            ? format(new Date(p.policyEndDate), "dd.MM")
                            : "—"}
                        </p>
                      </div>
                    </div>
