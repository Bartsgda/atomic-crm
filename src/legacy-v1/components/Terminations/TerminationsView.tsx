...
export const TerminationsView: React.FC<Props> = ({
  state,
  onNavigate,
  onRefresh,
}) => {
  ...
  const filtered = state.terminations.filter(
    (t) =>
  ...
  const handleUpdateLink = async (
    ...
    const updated = {
      ...record,
      [type === "local" ? "localPath" : "cloudLink"]: val,
    };
    ...
    reader.onload = async (event) => {
      try {
        ...
        for (const item of items) {
          const exists = currentState.terminations.find(
            (ex) => ex.id === item.id,
          );
          if (!exists) {
        ...
        alert(
          `Pomyślnie zaimportowano ${importedCount} nowych rekordów wypowiedzeń.`,
        );
        onRefresh();
      } catch (err) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <div className="flex items-center gap-4 mt-2">
            <p className="text-zinc-500 font-medium uppercase text-[10px] tracking-[0.2em]">
              Ewidencja wysłanych dokumentów
            </p>
            <button
        ...
        <div className="relative w-full md:w-96 group">
          <Search
            className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-red-500 transition-colors"
            size={20}
          />
          <input
            type="text"
                  ...
                  <td colSpan={6} className="px-8 py-24 text-center">
                    <div className="flex flex-col items-center gap-4 opacity-20">
                      ...
                      <p className="text-sm font-black uppercase tracking-widest text-zinc-400">
                        Brak zarejestrowanych wypowiedzeń
                      </p>
                    </div>
                  </td>
                </tr>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-zinc-400 font-medium">
                            {format(new Date(rec.sentAt), "HH:mm")}
                          </p>
                        </div>
                      </div>
                    </td>
                          ...
                          onClick={() => {
                            ...
                            if (c)
                              onNavigate("client-details", {
                                client: c,
                                highlightPolicyId: rec.policyId,
                              });
                          }}
                          className="text-left group/policy block"
                        >
                          <p className="text-[10px] font-black text-zinc-500 uppercase group-hover/policy:text-blue-600 transition-colors">
                            {rec.itemDescription}
                          </p>
                        </button>
                      </div>
                      ...
                      <div className="flex flex-col gap-2 max-w-xs">
                        ...
                        <div className="flex items-center gap-2 bg-zinc-50 dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700 rounded-lg px-2 py-1.5 group/input">
                          ...
                          <input
                            ...
                            value={rec.cloudLink || ""}
                            onChange={(e) =>
                              handleUpdateLink(rec, "cloud", e.target.value)
                            }
                          />
                          {rec.cloudLink && (
                            <a
