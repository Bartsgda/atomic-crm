...
export const AutoTester: React.FC<Props> = ({
  externalOpen,
  onClose,
  onRefresh,
}) => {
  ...
  // Esc zamyka (2026-07-27) — jak istniejący przycisk X (handleClose), bez blokady
  // na isRunning (X też nie jest wtedy disabled).
  useEffect(() => {
    if (!isOpen) return;
  ...
  const runAgentScenario = async (outcome: "SUCCESS" | "RECALC" | "GHOST") => {
    ...
    try {
      ...
      const dashBtn = Array.from(document.querySelectorAll("button")).find(
        (b) => b.textContent?.includes("Pulpit"),
      );
      ...
      await storage.addNote({
        ...
        content:
          "Telefon do klienta: Potwierdził bezszkodowość. Żona jako współwłaściciel.",
        tag: "ROZMOWA",
        createdAt: new Date().toISOString(),
      });
      ...
      if (outcome === "SUCCESS") {
        ...
        await storage.addNote({
          ...
          linkedPolicyIds: [policyId],
          content:
            "Za drogo. Klient ma ofertę w Link4 za 400zł. Sprawdzić MTU.",
          tag: "DECISION_PRICE",
          createdAt: new Date().toISOString(),
        });
        ...
        await storage.addNote({
          ...
          linkedPolicyIds: [policyId],
          content:
            "[ST: BRAK TEL] Próba kontaktu nieudana. Prosił o kontakt po 16:00.",
          tag: "STATUS",
          createdAt: new Date().toISOString(),
  ...
  const runSanityCheck = async () => {
    ...
    try {
      ...
      if (badIds.length > 0) {
        log(`❌ KRYTYCZNE: ${badIds.length} uszkodzonych ID!`);
        issues++;
    ...
    <div className="fixed bottom-4 right-4 z-[9999] w-80 bg-zinc-950/80 backdrop-blur-2xl border border-indigo-500/30 rounded-3xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] overflow-hidden font-mono text-xs animate-in slide-in-from-bottom-10 flex flex-col max-h-[80vh]">
      <div className="bg-gradient-to-r from-indigo-600 to-indigo-800 p-3 flex justify-between items-center flex-shrink-0">
        <span className="font-black text-white flex items-center gap-2">
          <Terminal size={14} className="text-indigo-200" /> SYMBBIOSIS AGENT
          v5.0
        </span>
      ...
      <div className="p-4 space-y-4 overflow-y-auto flex-1 custom-scrollbar">
        ...
        <div className="space-y-2">
          <p className="text-zinc-500 font-bold uppercase tracking-widest text-[9px] flex items-center gap-1">
            <Briefcase size={10} /> SCENARIUSZE SPRZEDAŻOWE
          </p>

          <button
            onClick={() => runAgentScenario("SUCCESS")}
            disabled={isRunning}
            className="tester-btn text-emerald-400 border-emerald-900/50 hover:bg-emerald-900/20"
          >
            <CheckCircle2 size={14} />
            <div>
              <div className="font-bold">Ścieżka: SUKCES</div>
              <div className="text-[8px] opacity-70">
                Klient &gt; Oferta &gt; Akceptacja &gt; Polisa
              </div>
            </div>
          </button>

          <button
            onClick={() => runAgentScenario("RECALC")}
            disabled={isRunning}
            className="tester-btn text-amber-400 border-amber-900/50 hover:bg-amber-900/20"
          >
            <RefreshCcw size={14} />
            <div>
              <div className="font-bold">Ścieżka: MARUDA</div>
              <div className="text-[8px] opacity-70">
                "Za drogo" &gt; Rekalkulacja &gt; Sukces
              </div>
            </div>
          </button>

          <button
            onClick={() => runAgentScenario("GHOST")}
            disabled={isRunning}
            className="tester-btn text-purple-400 border-purple-900/50 hover:bg-purple-900/20"
          >
            <Ghost size={14} />
            <div>
              <div className="font-bold">Ścieżka: DUCH</div>
              <div className="text-[8px] opacity-70">
                Brak kontaktu &gt; Przypomnienie
              </div>
            </div>
          </button>
        </div>
        ...
        <div className="space-y-2 pt-2 border-t border-zinc-800">
          <p className="text-zinc-500 font-bold uppercase tracking-widest text-[9px] flex items-center gap-1">
            <Database size={10} /> ADMIN TOOLS
          </p>

          <button
            onClick={runSanityCheck}
            disabled={isRunning}
            className="tester-btn text-blue-400"
          >
            <Stethoscope size={14} /> Diagnoza Bazy
          </button>

          <button
            onClick={runScenarioNuke}
            disabled={isRunning}
            className="tester-btn text-red-500 border-red-900 hover:bg-red-900/20"
          >
      ...
      <div className="bg-black p-2 h-32 overflow-y-auto border-t-2 border-yellow-600 text-[10px] text-zinc-400 font-mono flex-shrink-0">
        {logs.length === 0 && (
          <span className="opacity-50 italic">
            &gt; Gotowy. Wybierz scenariusz...
          </span>
        )}
        {logs.map((l, i) => (
          <div
            key={i}
            className="mb-1 border-b border-zinc-900 pb-0.5 last:border-0"
          >
            {l}
          </div>
        ))}
