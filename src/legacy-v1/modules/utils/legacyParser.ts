...
// Słowa które wyglądają jak tablice, ale nimi nie są
const PLATE_BLACKLIST = new Set([
  "NNW",
  "ASS",
  "TDI",
  "LPG",
...
function isValidPLPlate(candidate: string): boolean {
  const n = candidate.replace(/\s/g, "").toUpperCase();
  if (PLATE_BLACKLIST.has(n)) return false;
  if (n.length < 5 || n.length > 8) return false;
  // Musi mieć cyfry w sufiksie (min 1)
  const m = n.match(/^([A-Z]{2,3})([A-Z0-9]+)$/);
  if (!m) return false;
  return /\d/.test(m[2]);
}
...
export const PARSER_PATTERNS = {
  // --- AUTO ---
  // BUG #5 FIX v6.7: lookahead/lookbehind zamiast \b (podłoga _ jest word-char w PL plate context)
  // Wymuszamy obecność PRZYNAJMNIEJ JEDNEJ CYFRY (\d) w drugiej grupie.
  // Walidację PLATE_BLACKLIST robimy w parseAutoString przez isValidPLPlate().
  REG_NUMBER: /(?<![A-Z0-9])([A-Z]{2,3})\s?([A-Z0-9]*\d[A-Z0-9]*)(?![A-Z0-9])/i,

  VEHICLE_TYPE: {
    // FIX: Dodano marki ciężarówek oraz literówki (ciezarowy)
    CIEZAROWY:
      /ciężarowy|ciezarowy|dostawczy|furgon|iveco|transit|sprinter|ducato|boxer|jumper|master|movano|crafter|transporter|vito|caddy|man|scania|daf|actros|tgx|tgl|volvo fh|renault trucks/i,
    MOTOCYKL:
      /motocykl|motor|skuter|yamaha|honda cbr|kawasaki|suzuki gsx|ktm|junak|romet|vespa|piaggio/i,
...
export const parseAutoString = (raw: string) => {
  ...
  if (
    /(dom|mieszkanie|lokal|budowa|działka|nieruchomość|ul\.|al\.)/i.test(
      processingText,
    )
  ) {
    return {};
  ...
  // Typ
  if (PARSER_PATTERNS.VEHICLE_TYPE.CIEZAROWY.test(textLower))
    result.autoDetails.vehicleType = "CIEZAROWY";
  else if (PARSER_PATTERNS.VEHICLE_TYPE.MOTOCYKL.test(textLower))
    result.autoDetails.vehicleType = "MOTOCYKL";
  else if (PARSER_PATTERNS.VEHICLE_TYPE.QUAD.test(textLower))
    result.autoDetails.vehicleType = "QUAD";
  else if (PARSER_PATTERNS.VEHICLE_TYPE.CIAGNIK.test(textLower))
    result.autoDetails.vehicleType = "CIAGNIK";
  else if (PARSER_PATTERNS.VEHICLE_TYPE.PRZYCZEPA.test(textLower))
    result.autoDetails.vehicleType = "PRZYCZEPA";
  else if (PARSER_PATTERNS.VEHICLE_TYPE.AUTOBUS.test(textLower))
    result.autoDetails.vehicleType = "AUTOBUS";

  // Paliwo
  if (PARSER_PATTERNS.FUEL.DIESEL.test(textLower))
    result.autoDetails.fuelType = "DIESEL";
  else if (PARSER_PATTERNS.FUEL.LPG.test(textLower))
    result.autoDetails.fuelType = "LPG";
  else if (PARSER_PATTERNS.FUEL.HYBRYDA.test(textLower))
    result.autoDetails.fuelType = "HYBRYDA";
  else if (PARSER_PATTERNS.FUEL.ELEKTRYK.test(textLower))
    result.autoDetails.fuelType = "ELEKTRYK";
  else if (PARSER_PATTERNS.FUEL.BENZYNA.test(textLower))
    result.autoDetails.fuelType = "BENZYNA";
  ...
  let cleanBrand = processingText
    .replace(
      /samochód[\s]+|pojazd[\s]+|motocykl[\s]+|autobus[\s]+|przyczepa[\s]+|przyczepka[\s]+|quad[\s]+|ciężarowy[\s]+|ciezarowy[\s]+/gi,
      " ",
    )
    .replace(
      /oc\/ac|oc|ac|nnw|ass|szyby|pakiet|kosztorys|serwis|pełen|opony|samo ac|komunikacyjne/gi,
      " ",
    )
...
export const parseHomeString = (raw: string) => {
  ...
  else if (PARSER_PATTERNS.HOME_TYPE.LETNISKOWY.test(text))
    result.objectType = "LETNISKOWY";
  else if (PARSER_PATTERNS.HOME_TYPE.MIESZKANIE.test(text))
    result.objectType = "MIESZKANIE";
