...
const DEFAULT_CHECKLISTS: ChecklistTemplates = {
  ...
  PODROZ: [{ id: "zakres", label: "Zakres Terytorialny", isRequired: true }],
};
...
class StorageManager {
  ...
  private syncInsurers(policies: Policy[], existingInsurers: string[]) {
    ...
    const validExisting = (existingInsurers || []).filter(
      (x) =>
        x &&
        typeof x === "string" &&
        x.trim().length > 1 &&
    ...
    if (storedData) {
      try {
        ...
        if (parsed && Array.isArray(parsed.clients)) {
          ...
          const syncedInsurers = this.syncInsurers(
            loadedPolicies,
            loadedInsurers,
          );
          ...
          this.state = {
            ...
            notifications: Array.isArray(parsed.notifications)
              ...
              : [],
            terminations: Array.isArray(parsed.terminations)
              ? parsed.terminations
              : [],
            ...
            checklistTemplates:
              ...
              typeof parsed.checklistTemplates === "object"
                ? parsed.checklistTemplates
                : DEFAULT_CHECKLISTS,
            insurers: Array.isArray(syncedInsurers)
              ? syncedInsurers
              : DEFAULT_INSURERS,
            insurerConfigs:
              parsed.insurerConfigs && typeof parsed.insurerConfigs === "object"
                ? parsed.insurerConfigs
                : {},
  ...
  async addClient(client: Client): Promise<AppState> {
    const exists = this.state.clients.find(
      (c) =>
        (c.pesel && client.pesel && c.pesel === client.pesel) ||
        c.id === client.id,
    );
    ...
    this.log(
      "CREATE",
      "CLIENT",
      `Utworzono klienta: ${client.lastName} ${client.firstName}`,
      client.id,
    );
    this.save();
    return this.getState();
  ...
  async updateClient(updatedClient: Client): Promise<AppState> {
    ...
    this.log(
      "UPDATE",
      "CLIENT",
      `Aktualizacja danych: ${updatedClient.lastName}`,
      updatedClient.id,
    );
    this.save();
    return this.getState();
  ...
  async addPolicy(policy: Policy): Promise<AppState> {
    ...
    if (
      policy.insurerName &&
      !this.state.insurers.includes(policy.insurerName)
    ) {
    ...
    this.log(
      "CREATE",
      "POLICY",
      `Dodano polisę ${policy.type}: ${policy.vehicleBrand || ""}`,
      policy.id,
    );
    this.save();
    return this.getState();
  ...
  async updatePolicy(updatedPolicy: Policy): Promise<AppState> {
    const oldPolicy = this.state.policies.find(
      (p) => p.id === updatedPolicy.id,
    );
    ...
    if (
      ...
      !this.state.insurers.includes(updatedPolicy.insurerName)
    ) {
      this.state.insurers = [
        ...this.state.insurers,
        updatedPolicy.insurerName,
    ...
    if (oldPolicy && oldPolicy.stage !== updatedPolicy.stage) {
      this.log(
        "UPDATE",
        "POLICY",
        `Zmiana etapu: ${oldPolicy.stage} -> ${updatedPolicy.stage}`,
        updatedPolicy.id,
      );
    } else {
      this.log(
        "UPDATE",
        "POLICY",
        `Edycja polisy ${updatedPolicy.policyNumber}`,
        updatedPolicy.id,
      );
    }
    this.save();
  ...
  async deletePolicy(id: string): Promise<AppState> {
    ...
    this.log(
      "DELETE",
      "POLICY",
      `Przeniesiono do kosza (Archiwum) polisę ${p.insurerName} ${p.vehicleBrand}`,
      id,
      p,
    );
    this.save();
  ...
  async deleteClient(id: string): Promise<AppState> {
    ...
    const associatedPolicies = this.state.policies.filter(
      (p) => p.clientId === id,
    );
    const associatedNotes = this.state.notes.filter((n) => n.clientId === id);
    const associatedTerminations = this.state.terminations.filter(
      (t) => t.clientId === id,
    );
    ...
    this.state.terminations = this.state.terminations.filter(
      (t) => t.clientId !== id,
    );
    ...
    this.log(
      "DELETE",
      "CLIENT",
      `Zarchiwizowano klienta oraz ${associatedPolicies.length} polisy i ${associatedNotes.length} notatki: ${client.lastName} ${client.firstName}`,
      id,
    );
    this.save();
    return this.getState();
  ...
  async restoreFromTrash(id: string): Promise<AppState> {
    ...
    if (item.type === "POLICY") {
      ...
      this.log(
        "RESTORE",
        "POLICY",
        `Przywrócono z kosza polisę: ${item.data.policyNumber}`,
        id,
      );
      ...
      if (terminations)
        this.state.terminations = [...this.state.terminations, ...terminations];

      this.log(
        "RESTORE",
        "CLIENT",
        `Przywrócono z archiwum klienta: ${client.lastName} ${client.firstName}`,
        id,
      );
  ...
  async purgeFromTrash(id: string): Promise<AppState> {
    ...
    this.log(
      "DELETE",
      "SYSTEM",
      `Trwale usunięto z kosza: ${item.type} (ID: ${id})`,
      id,
    );
    this.save();
    return this.getState();
  ...
  async addNote(note: ClientNote): Promise<AppState> {
    ...
    this.log(
      "CREATE",
      "NOTE",
      `Notatka [${note.tag}]: ${note.content.substring(0, 20)}...`,
      note.id,
    );
    this.save();
    return this.getState();
  ...
  async addTerminationRecord(record: TerminationRecord): Promise<AppState> {
    ...
    this.log(
      "CREATE",
      "TERMINATION",
      `Zarejestrowano wypowiedzenie dla ${record.clientName}`,
      record.id,
    );
    this.save();
    return this.getState();
  ...
  async deleteTerminationRecord(id: string): Promise<AppState> {
    ...
    this.state.policies = this.state.policies.map((p) => {
      if (p.terminationId === id)
        return { ...p, isTerminationSent: false, terminationId: undefined };
      return p;
    });
    this.log(
      "DELETE",
      "TERMINATION",
      `Usunięto wpis z rejestru wypowiedzeń`,
      id,
    );
    this.save();
    return this.getState();
  ...
  async updateTerminationRecord(updated: TerminationRecord): Promise<AppState> {
    this.state.terminations = this.state.terminations.map((t) =>
      t.id === updated.id ? updated : t,
    );
    this.log(
      "UPDATE",
      "TERMINATION",
      `Aktualizacja linków w wypowiedzeniu`,
      updated.id,
    );
    this.save();
    return this.getState();
  ...
  async updateSubAgent(updated: SubAgent): Promise<AppState> {
    ...
    this.log(
      "UPDATE",
      "SUB_AGENT",
      `Zaktualizowano pośrednika: ${updated.name}`,
      updated.id,
    );
    this.save();
    return this.getState();
  ...
  async deleteSubAgent(id: string): Promise<AppState> {
    this.state.subAgents = (this.state.subAgents || []).filter(
      (a) => a.id !== id,
    );
  ...
  async addNotification(notif: Notification): Promise<AppState> {
    this.state.notifications = [notif, ...this.state.notifications].slice(
      0,
      50,
    );
    this.save();
  ...
  async clearAllData(): Promise<void> {
    ...
    this.log(
      "DELETE",
      "SYSTEM",
      "Nuclear Reset: Wszystkie dane lokalne zostały usunięte.",
    );
    this.save();
  }
}
